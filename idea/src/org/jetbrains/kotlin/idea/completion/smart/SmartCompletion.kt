...
class SmartCompletion(
    ...
    private fun buildForLoopRangePosition(expressionWithType: JetExpression, receiver: JetExpression?): Result? {
        ...
        return buildResultByTypeFilter(expressionWithType, receiver) { iterableDetector.isIterable(it.type) }
    }
    ...
    private fun buildForInOperatorArgument(expressionWithType: JetExpression, receiver: JetExpression?): Result? {
        ...
        return buildResultByTypeFilter(expressionWithType, receiver) { detector.hasContains(it) }
    }

    private fun buildResultByTypeFilter(position: JetExpression, receiver: JetExpression?, typeFilter: (FuzzyType) -> Boolean): Result {
